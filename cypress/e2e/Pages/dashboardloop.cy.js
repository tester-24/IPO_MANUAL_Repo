...
//check for the whole table contains SME ipo 
 let containsSME = false;
 cy.xpath('/html/body/app-root/app-layout/app-maincontent/app-ipolist/section[1]/div/div[3]/div/kendo-grid/div/kendo-grid-list/div/div[1]/table/tbody/tr').each(($row)=>{
  cy.wrap($row).invoke('text').then((rowText) => {
    if (rowText.includes('SME')) {
      containsSME = true;  // Set to true if 'SME' is found in any row
    }
 })
 }).then(() => {
  // After looping through all the rows, check if 'SME' was found
  if (containsSME) {
    cy.log('The table contains "SME", not clicking the bid button');
  } 
  else {
    cy.log('The table does not contain "SME", clicking the bid button');
    cy.get('[data-kendo-grid-item-index="0"] > [data-kendo-grid-column-index="5"] > .bid-btn').should('be.visible')
    cy.get('[data-kendo-grid-item-index="0"] > [data-kendo-grid-column-index="5"] > .bid-btn').click() // Adjust the selector for your bid button
  }
});
//  if(cy.xpath('/html/body/app-root/app-layout/app-maincontent/app-ipolist/section[1]/div/div[3]/div/kendo-grid/div/kendo-grid-list/div/div[1]/table/tbody/tr[1]/td[2]/div/span').should('have.text', ''))
...
// else
// {
//   cy.log('Not allow to bid it its SME ipo ')
// }     
